...
# CSVファイルを読み込み
df = pd.read_csv(input_csv_file, encoding='utf-8', dtype=str)
print(df.head(5))
...
def format_mag(mag_str):
    invalid_strings = ['-1', '-2', '-3', '-4', '-5', '-6', '-7', '-8', '-9', 'A0', 'A1',
                       'A2', 'A3', 'A4', 'A5', 'A6', 'A7', 'A8', 'A9', 'B0', 'C0']
    if mag_str not in invalid_strings and mag_str != '  ':
...
df[['地震ID', 'レコード種別', 'DateTime', 'UnixTime', 'Latitude', 'Longitude',
    '深さ(km)', 'マグニチュード1', 'マグニチュード2', '最大震度', '大地域区分番号', '小地域区分番号', '震央地名', '観測点数', '震源決定フラグ']].to_csv(output_csv_file, index=False)
